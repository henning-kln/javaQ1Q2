...
public class DijkstraTest {
   ...
   public List<DijkstraInfo> dijkstraAlgorithmus(String sStart){
       ...
       while(!gelbeListe.isEmpty()) {
    	   gelbeListe.toFirst();
    	   DijkstraInfo dij = gelbeListe.getContent();
    	   if  (dij.getVorgaenger() != null) {
    		   graph.getEdge(dij.getVertex(), dij.getVorgaenger()).setMark(true);
    	   }
    	   
    	   roteListe.append(dij);
    	   dij.getVertex().setMark(true);
    	   gelbeListe.remove();
    	   List<Vertex> n = graph.getNeighbours(dij.getVertex());
    	   for (n.toFirst();n.hasAccess();n.next()) {
    		   if(!n.getContent().isMarked()) {
    			   DijkstraInfo n_dij = new DijkstraInfo(n.getContent(), graph.getEdge(n.getContent(), dij.getVertex()).getWeight()+dij.getDistanz(), dij.getVertex());
    		   		inGelbeListeUpdaten(n_dij);
    		   }
    	   }
   ...
   private void inGelbeListeUpdaten(DijkstraInfo dInfo) {
    ...
    public void initMap(){
			...
			Vertex dresden = new Vertex("Dresden");
			graph.addVertex(dresden);
			Vertex chemnitz = new Vertex("Chemnitz");
			graph.addVertex(chemnitz);
			Vertex erfurt = new Vertex("Erfurt");
			graph.addVertex(erfurt);
			Vertex rostock = new Vertex("Rostock");
			graph.addVertex(rostock);
			Vertex magdeburg = new Vertex("Magdeburg");
			graph.addVertex(magdeburg);
			Vertex badHonnef = new Vertex("Bad Honnef");
			graph.addVertex(badHonnef);
            ...
            graph.addEdge(new Edge(dresden, berlin, 196));
            graph.addEdge(new Edge(chemnitz, dresden, 102));
            graph.addEdge(new Edge(chemnitz, leipzig, 72));
            graph.addEdge(new Edge(leipzig, erfurt, 156));
            graph.addEdge(new Edge(erfurt, kassel, 141));
            graph.addEdge(new Edge(rostock, berlin, 230));
            graph.addEdge(new Edge(rostock, hamburg, 188));
            graph.addEdge(new Edge(magdeburg, berlin, 159));
            graph.addEdge(new Edge(magdeburg, hannover, 148));
            graph.addEdge(new Edge(badHonnef, koeln, 37));
            graph.addEdge(new Edge(badHonnef, bonn, 15));
            graph.addEdge(new Edge(bonn, frankfurt, 155));
